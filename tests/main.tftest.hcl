...
run "check_scope_for_role_assignment" {
  command = plan
...
run "check_outputs" {
  ...
  assert {
    condition     = output.byoc_setup_details.bucket_name == azurerm_storage_account.terraform_state.name
    error_message = "Output bucket name does not match storage account name"
  }

  assert {
    condition     = output.byoc_setup_details.subscription_id == var.subscription_id
    error_message = "Output subscription ID does not match input"
  }

  assert {
    condition     = output.byoc_setup_details.azure_client_id == azuread_application_registration.github_actions.client_id
    error_message = "Output Azure client ID does not match application registration client ID"
  }

  assert {
    condition     = output.byoc_setup_details.tenant_id == data.azuread_client_config.current.tenant_id
    error_message = "Output tenant ID does not match current tenant ID"
  }

  assert {
    condition     = output.byoc_setup_details.application_name == azuread_application_registration.github_actions.display_name
    error_message = "Output application name does not match application registration display name"
  }
}
